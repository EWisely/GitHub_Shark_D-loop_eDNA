#export final REPLICATE_AGGREGATED dataset for phyloseq (ASVs/haplotype can be merged in phyloseq) 

library(dplyr)

#reads
df.FINAL.reads_Climb_22_23_hap_matched<- as.data.frame(Climb_22_23_hap_matched$reads)
write.csv(df.FINAL.reads_Climb_22_23_hap_matched, "data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_reads_22-23_Climb2_amplified_hap_matched_ASVs.csv")

#motus
df.FINAL.motus_Climb_22_23_hap_matched<- as.data.frame(Climb_22_23_hap_matched$motus)
write.csv(df.FINAL.motus_Climb_22_23_hap_matched, "data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_ASVs_22-23_Climb2_amplified_hap_matched_ASVs.csv")

#samples
df.FINAL.samples_Climb_22_23_hap_matched<- as.data.frame(Climb_22_23_hap_matched$samples)
write.csv(df.FINAL.samples_Climb_22_23_hap_matched, "data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_samples_22-23_Climb2_amplified_hap_matched_ASVs.csv")

#pcrs (the metadata in here should be unnecessary at this point)
df.FINAL.pcrs_Climb_22_23_hap_matched<- as.data.frame(Climb_22_23_hap_matched$pcrs)
write.csv(df.FINAL.pcrs_Climb_22_23_hap_matched, "data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_pcrs_22-23_Climb2_amplified_hap_matched_ASVs.csv")


#Save R Global Environment
save.image(file = "data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_22-23_Climb2_amplified_hap_matched_ASVs.RData")
...
load("data/22-23_Climb2_amplified/05_MetabaR_filtered/post_MetabaR_22-23_Climb2_amplified_hap_matched_ASVs.RData")
#otu table
...
clean.otu.df<-as.data.frame(t(Climb_22_23_hap_matched$reads))
...
taxaId<-df.FINAL.motus_Climb_22_23_hap_matched$TAXID
hapID<-df.FINAL.motus_Climb_22_23_hap_matched$annot_hap
#uncomment the 2 lines below to make a taxa table for just the final aggregated data
#df.FINAL.motus_Climb_22_23_hap_matched<-rownames_to_column(df.FINAL.motus_GAL21_Diet_Combined_aggregated, var = "id")
...
rownames(taxa.df)<-df.FINAL.motus_Climb_22_23_hap_matched$id

#backup plan for haplotypes instead of full biodiversity dataset:
haplo.df<- df.FINAL.motus_Climb_22_23_hap_matched %>% select(TAXID, SCIENTIFIC_NAME, BEST_IDENTITY, annot_hap, annot_qual, annot_pctid)
